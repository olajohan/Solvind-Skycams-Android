...
import com.solvind.skycams.app.domain.usecases.ActivateAlarmUseCase
...
class ActivateAlarmGetAllAlarmsFlowUseCaseTest {
    ...
    fun setup() {
        ...
        activateAlarmUseCase = ActivateAlarmUseCase(repo, getAlarmUseCase, testDispatcher)
    }

    @Test
    fun activateAlarm_emptySkycamKey_returnsResourceError() = runBlockingTest {
        val TEST_SKYCAM_KEY = ""
        val params = ActivateAlarmUseCase.Params(TEST_SKYCAM_KEY)
    ...
    fun setAlarmStatus_isActiveTrue_keepOtherParamsTheSameAndIsActiveTrue() = runBlockingTest {
        ...
        val TEST_CURRENT_ALARM = Alarm(TEST_SKYCAM_KEY, Instant.now().epochSecond + TimeUnit.MINUTES.toSeconds(30L), false)
        val setAlarmParams = ActivateAlarmUseCase.Params(TEST_SKYCAM_KEY)
        `when`(repo.getAlarm(TEST_SKYCAM_KEY)).thenReturn(Resource.Success(TEST_CURRENT_ALARM))
        activateAlarmUseCase.run(setAlarmParams)

        verify(repo).setAlarm(eq(TEST_SKYCAM_KEY), eq(TEST_CURRENT_ALARM.alarmAvailableUntilEpochSeconds), eq(true))
    }
